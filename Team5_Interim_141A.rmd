...
We will determine if there is correlation between brain regions by computing the Spike Time Tiling Coefficient (STTC), 
which is a measure that computes the correlation between a pair of spike trains. STTC is a method that was proposed by Catherine S. Cutts and Stephen J. Eglen in the paper "Detecting Pairwise Correlations in Spike Trains: An Objective Comparison of Methods and Application to the Study of Retinal Waves"
in the Journal of Neuroscience. 

We will use the STTC to produce a correlogram. Suppose we have two spike trains A and B. For a given value tau, we will cross
correlate A and B by adding tau to the spike time of B. This will help us detect correlations when temporal delays occur. 

Additionally, we will compute the pairwise STTC, and create a lattice plot to demonstrate the pairwise correlations as a function of distance. 
