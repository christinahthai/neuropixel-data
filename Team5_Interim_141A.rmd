...
### Extract the spike train for region 1
...
```{r, echo = FALSE, eval= TRUE}
### Extract the spike train for reg 2
spikes_this_region = exp_data[[brain_regions[2]]]
...
for(i_neuron in 1:n_neurons){
  spk_times =which(spikes_this_region[i_neuron,time_range[1]:time_range[2]]>0);
  if (length(spk_times)>0){
    points(y=rep(i_neuron,length(spk_times)),x=spk_times+time_range[1]-1,col="#000000",pch='.',cex=1)
  }
}
```

```{r, echo = FALSE, eval= TRUE}
### Extract the spike train for reg 3
spikes_this_region = exp_data[[brain_regions[3]]]
